...
Shader "FX/Holo Dissolve" {
    Properties {
      _MainTex ("Dissolve Tex", 2D) = "white" {}
	  ...
	  _Bp("Normal Strength", Range(0.0,2.0)) = 1.0
		_Pos("Dissolve Amount", Range(0.0,1.0)) = 0.5
    }
    ...
    cull off
      ...
      void surf (Input IN, inout SurfaceOutput o) {
         ...
         half backside = 0;
		  ...
		  float dissolve = tex2D(_MainTex, IN.uv_MainTex) -(_Pos*1.2f-0.1f);
		  clip(dissolve);
		  bright += lerp(0, 20, 1-saturate(dissolve / 0.1));
          o.Emission = _Color.a * 0.4 * lerp((_RimColor.rgb * pow (rim, _RimPower) * 20),  _Color  * 3 - (backside * 0.2), -(_RimColor.rgb * pow (rim, _RimPower)) + 1) * bright  * _Brightness * IN.color;
