    ...
    public class BuggyBuddy : MonoBehaviour
    {
        ...
        */
        ...
        private void Update()
        {
            ...
            mvol = Mathf.Lerp(mvol, Mathf.Pow((fVol / maxTorque), 0.8f) * Mathf.Lerp(0.4f, 1.0f, (Mathf.Abs(wheels[2].rpm) / 200)) * Mathf.Lerp(1.0f, 0.5f, handBrake), Time.deltaTime * 9);
            ...
            if (fleeTarget != null)
            {
                fleeTarget.active = Mathf.Abs(speed) > 5;
            }
            ...
            for (int i = 0; i < wheelRenders.Length; i++)
            {
                ...
                if (wheelRenders[i] != null && wheels[0].enabled)
                {
                    Quaternion q;
                    Vector3 p;
