...
public class SpaceJoeJeff : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (skinPallette.Length > 0)
        {
            skinColor = skinPallette[Random.Range(0, skinPallette.Length)];
    ...
    const float damageEnergyThreshold = 1000;
    void OnCollisionEnter(Collision col)
    {
        if (col.collider.attachedRigidbody != null)
        {
            if(col.collider.attachedRigidbody.mass * col.collider.attachedRigidbody.velocity.magnitude > damageEnergyThreshold)
            {
                Die(col.contacts[0].normal * col.collider.attachedRigidbody.velocity.magnitude, col.contacts[0].point);
            }
        }
    }
